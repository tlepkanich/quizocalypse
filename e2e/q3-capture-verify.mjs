// WIS-025 capture integration matrix; extends the original rg-wiring regression checks.
...
const SHOTS = "/tmp/wis025-capture-shots";
if (!KEY) {
...
const originalShop = await prisma.shop.findUnique({where:{id:quiz.shopId},select:{aiRecCopyEnabled:true}});
...
try {
  ...
  seeded = true;
  await prisma.shop.update({where:{id:quiz.shopId},data:{aiRecCopyEnabled:true}});
  ...
  let aiRequests = 0;
  await ctx.route('**/rec-copy',async route=>{aiRequests++;await route.fulfill({status:200,contentType:'application/json',body:JSON.stringify({ok:true,copy:'Matched using your quiz answers.'})});});
  ...
  await shopper.locator('input[type="email"]').first().fill("wis025-probe@example.com");
  ...
  const capRow = await prisma.emailCapture.findFirst({
    where: { quizId: QUIZ, email: "wis025-probe@example.com" },
    orderBy: { capturedAt: "desc" },
  });
  ok("EmailCapture row stores marketingConsent:true", capRow?.marketingConsent === true);

  // WIS-025 — the four newly available forms use the real /captures route.
  for (const placement of ['before','inline']) for (const mode of ['checkbox','notice']) {
    const label = `${placement}-${mode}`;
    const next = mkDoc({capturePlacement:placement,captureInlineOn:placement==='inline'?true:undefined,
      captureEmail:true,capturePhone:true,captureTermsOn:true,captureTermsMode:mode,smsConsentMode:mode,
      captureRequired:true,captureCta:'Save details',loadingOn:false,
      captureTermsText:mode==='notice'?'Read our {terms} and {privacy}.':'I accept these terms.',
      smsConsentText:'I agree to receive SMS messages.', termsUrl:'/policies/terms-of-service',termsLabel:'Our terms'});
    await prisma.quiz.update({where:{id:QUIZ},data:{draftJson:next}});
    const response=await publish(next);if(!response.ok())throw new Error(`Publish ${label} failed`);
    const form=await ctx.newPage();form.on('pageerror',e=>out.pageErrors.push(String(e).slice(0,200)));
    const aiBefore = aiRequests;
    if(placement==='inline'&&mode==='notice')await form.setViewportSize({width:390,height:844});
    await walkToCapture(form);
    if(placement==='inline') {
      await waitForReveal(form);
      ok(`${label}: results visible before capture`,await form.getByRole('button',{name:/start over/i}).isVisible());
      ok(`${label}: AI copy request does not wait for capture`, aiRequests>aiBefore);
    }
    await form.locator('input[type="email"]').fill(`wis025-probe-${label}@example.com`);
    await form.locator('input[type="tel"]').fill('15555550123');
    const save=form.getByRole('button',{name:'Save details',exact:true});
    if(mode==='checkbox') {
      ok(`${label}: consent gates submission`,await save.isDisabled());
      await form.getByLabel('I accept these terms.',{exact:true}).check();
      ok(`${label}: SMS remains independent`,await save.isDisabled());
      await form.getByLabel('I agree to receive SMS messages.',{exact:true}).check();
    } else {
      ok(`${label}: no affirmative boxes`,await form.locator('input[type="checkbox"]').count()===0);
      ok(`${label}: substituted shop policy URL`,(await form.getByRole('link',{name:'Our terms',exact:true}).first().getAttribute('href'))?.startsWith('https://'));
    }
    ok(`${label}: no horizontal overflow`,await form.evaluate(()=>document.documentElement.scrollWidth<=window.innerWidth+1));
    await form.screenshot({path:`${SHOTS}/${label}.png`,fullPage:true});
    const savedResponse=form.waitForResponse(r=>r.url().includes('/captures')&&r.request().method()==='POST');
    await save.click();ok(`${label}: capture accepted`,(await savedResponse).status()===202);
    if(placement==='inline')await form.getByRole('status').filter({hasText:'Your details have been saved.'}).waitFor();
    else await waitForReveal(form);
    const row=await prisma.emailCapture.findFirst({where:{quizId:QUIZ,email:`wis025-probe-${label}@example.com`},orderBy:{capturedAt:'desc'}});
    ok(`${label}: consent evidence and server timestamp stored`,row?.consentEvidence?.terms?.mode===mode&&row?.consentEvidence?.terms?.checked===(mode==='checkbox')&&row?.consentEvidence?.sms?.checked===(mode==='checkbox')&&row?.consentRecordedAt instanceof Date);
    ok(`${label}: stored terms are the displayed sentence`,row?.consentEvidence?.terms?.text===(mode==='notice'?'Read our Our terms and Privacy Policy.':'I accept these terms.'));
    await form.close();
  }
  ...
  await shopper2.locator('input[type="email"]').first().fill("wis025-probe2@example.com");
  ...
  // ── restore ───────────────────────────────────────────────────────────────
  if (seeded) {
    await prisma.shop.update({where:{id:quiz.shopId},data:{aiRecCopyEnabled:originalShop.aiRecCopyEnabled}});
    await prisma.quiz.update({
      where: { id: QUIZ },
    ...
    await prisma.emailCapture.deleteMany({
      where: { quizId: QUIZ, email: { startsWith: "wis025-probe" } },
    });
