...
| `qrtzg1-spec-verify.mjs` | QRTZ-G1 speculative question-gen prefetch: the real buckets page's 5s settle writes `build_session.speculative` (stage NEVER leaves grouping, draft/name untouched); same-signature ping = cached, pool change supersedes, failed = silent tombstone, over-budget = silent skip; Continue applies a READY result instantly (fail mode: fresh path + marker clear at $0; happy mode: question nodes in one intent round-trip <8s, AiUsage delta 0 after Continue) | LOCAL build + `cmr7khgd5…` (fail: `ANTHROPIC_API_KEY=` blank server; happy: real AI, ~4 calls) | prisma seed/restore (doc + name + categories + shop + AiUsage); waits out the kicked chain before restoring |
| `rg-wiring-verify.mjs` | rg-wiring (2026-08-18): the guided Results settings reach the published /q — capture CTA/skip/consent row/policy links, merchant stepped loading, perRow grid, desc overrides, extras shelf, `marketing_consent` on the capture row — PLUS the negative control (empty global = the exact legacy posture) | LOCAL build + `cmr7khgd5…` | prisma seed/restore (draft + published + version + categories); deletes its probe capture rows |
| `q3-questions-verify.mjs` | WIS-025 walkthrough: real autosave, answer editing/reorder, composer/delete, review gate, capture invariants and to-logic seam | LOCAL build + `cmr7khgd5…` | prisma draft/category snapshot → restore |
| `q3-logic-verify.mjs` | Preserved Logic workspace and style coverage; enters through the to-logic HTTP intent independently of Questions UI | LOCAL build + `cmr7khgd5…` | prisma draft/category snapshot → restore |
| `q3-capture-verify.mjs` | Before/inline × checkbox/notice real submissions, consent evidence/timestamp, plus original Results wiring and absent-field controls | LOCAL build + `cmr7khgd5…` | draft/published/category snapshot → restore; probe captures deleted |
| `create-rule-verify.mjs` | Create-a-rule ONE-SCREEN modal (create-rule handoff §7): 1,330px shell, one ROW per question with 130×62 buckets, constant 124px operator column (`is / is not`, `any of / all of` stated-not-offered on single-select), strip/focus position holding, BOTH bands — funnel `flow="onboarding"` (Your recommendations, `N/M have a rule`, uncovered-first amber/green, upward peek that never selects, `+ Add something else` UNDER the groups) and builder (What the quiz shows, tabs + search, no amber/green, filter never hides a pick) — `Create & add another` commit/toast/reset + coverage moves, ensure-targets failure keeps the draft, edit mode footer | LOCAL build + `cmr7khgd5…` | prisma snapshot → byte-for-byte restore (draftJson + quiz-scoped Category rows) in `finally` |
| `recs-step-verify.mjs` | Recommendations step — the Step-1 tweaks (rev-2 handoff): four tabs printing their OWN picks (a tab is a view — no TabLockModal), the narrowing line + explainer, sort / status / one-button toolbar, the 25-row window that loads on scroll (+ quiet keyboard load-more), the footer ledger (`Showing 25 of 68 active · 55 draft, 1 archived not shown` → All statuses), one-line rows with the count / variant list as the control (per-variant stock modal), the Results rail (composition sub-line, Groups/Products sections, deliverable copy, flash on add, ✕ never opens the preview, rows open the preview), the FROZEN selected-first order, a HETEROGENEOUS selection persisted as Category rows, the AI Picks pill → dialog → TYPE-scoped Apply (the other types survive) → Undo, Select all / Clear all confirms scoped to the search, and the Custom tab creating a group through the shared wizard (shop-global row + a `group` bucket row keyed on its id) | LOCAL build + `cms4squp0…` (goal-first READY at grouping) | prisma snapshot → restore (draftJson + quiz Category rows); deletes any shop-global group it created |
| `logic-widget-verify.mjs` | QWIDGET — the Logic step's question widget (Attributes + Rules): Format B rail with role tags, the pane's text-with-caret role control, the RECOMMENDATIONS FROM STEP 1 tray (the quiz's step-1 groups surface with real thumbnails; used ones grey + slide last), four-column rows with the count inside the chip (products popover survives), the cell as the control (click / Enter), the multi-map picks picker (checkboxes that stay open, › drill-in, writes on click, ADDS; QWIDGET-M), arm-and-place with `Add {name}` / `{name} is already here`, the per-chip ×, the staged narrows picker (Cancel writes nothing), inert info rows, and decision 2 (a multi-select promoted to decider from the role menu) | LOCAL build + `cmr7khgd5…` | prisma snapshot → byte-for-byte draft restore |
...
| `rs-verify.mjs` | Step-1 Recommendations spec (copy, rail, no "bucket" leakage) — **KNOWN-RED since the Step-1 tweaks** (drives the retired switch-confirm / "Switch types"; a tab is a view now — see `recs-step-verify.mjs`) | LIVE, `cmqqcb0ao…` pin | restore |
| `shape-scope-verify.mjs` | Funnel AI grounds in CHOSEN buckets at both gen layers (65c55a5) | LIVE; real AI | restore |
| `s12-reaudit-verify.mjs` | **KNOWN-RED since the Step-1 tweaks** (asserts the retired amber warn, switch-confirm and count-pill) — AUDIT-18 Step-1 + Shape vs the design sub-files (stacked stepper geometry, AI-tip pill/first-visit, amber warn, products-modal meta, shape cards/film/leave-confirm) | LOCAL build + `cmr7khgd5…` | prisma seed/restore (doc + categories) |
