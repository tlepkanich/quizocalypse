...
| `logic-widget-verify.mjs` | QWIDGET — the Logic step's question widget (Attributes + Rules): Format B rail with role tags, the pane's text-with-caret role control, the RECOMMENDATIONS FROM STEP 1 tray (the quiz's step-1 groups surface with real thumbnails; used ones grey + slide last), four-column rows with the count inside the chip (products popover survives), the cell as the control (click / Enter), the single-target picks picker (radios, › drill-in, writes on click, REPLACES), arm-and-place with `Replace {old} with {new}`, the clear ×, the staged narrows picker (Cancel writes nothing), inert info rows, and decision 2 (a multi-select promoted to decider from the role menu) | LOCAL build + `cmr7khgd5…` | prisma snapshot → byte-for-byte draft restore |
