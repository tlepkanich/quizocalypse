...
| R10 · Migration, probes, sweep | (uncommitted) | 🚧 **sweep ✅** — vocabulary sweep clean (the Logic UI uses result/rule/decider/override/dead-end/fallback; no legacy points-model terms) · architecture cross-link added to `docs/quizzy-v1-architecture.md` (QZY-R row + probe inventory) · this status table current. Probes per acceptance line live: `qzy-r2…r9b-verify.mjs`. **DEPLOY ⏳** — the full batch (`1156d8c`…) ships via CI-on-push (gates + smoke + auto-rollback); byte pin `c02ccaec98a0fe9e` verified post-deploy. |
