...
| R7 · Element-granular inspector + block polish | R7-1 `90b3149` · R7-2 `3713faf` | 🚧 **R7-1 ✅** block-editor polish (§7.1/§7.3): image block gains upload via the R4 `MediaPicker` (was URL-only); **divider** gains thickness + colour (a `BlockFields` case existed in schema/runtime but had no editor); **progress** gains fill + track colour inputs. All builder-only — the runtime already consumes these fields, so no schema/runtime change (byte-safe). Gate 1391; builderv3 76/76. **R7-2 ✅** (`3713faf`, first RUNTIME touch of R7, byte-safe by construction): video **captions** VTT `<track>` (null when unset → `<video>` byte-identical) + editor field; progress **radius** (unset → the prior pill 999) + **show_count** "N of M" (unset → the bar/dots node exactly as before, no `flex` leak) + radius scrub/count toggle. Gate 1391; builderv3 76/76. **R7-3 (remaining):** Next-button size + radius (§7.2) · Layers drag-reorder (BT4) · the element-granular selection map (BT1 §2 — XL; likely scoped-down + documented). |
