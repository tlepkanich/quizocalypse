...
| R9 · Table tab + override-writes-a-rule | R9-1 `279c496` · R9-2 `84d82bc` | ✅ **COMPLETE** — **R9-1 ✅** the **Table tab** (`LogicTableTab`): collapsed = one row per result (result chip · path count · ✓/⚠N status); expand → per-path rows with a COLUMN per question, a skipped question rendering `–` (the complement of the Paths lanes); answer cells navigate to the Map. Probe `qzy-r9-verify.mjs` 11/11 + screenshot. **R9-2 ✅** (LV4) result **override → writes a path-signature rule**: `createRuleWithConditions` (multi-condition generalization of `createRuleWithCondition`, 3 unit tests); a per-path "override" affordance (gated on categories) writes "If Q1 is X AND Q2 is Y → target" into the SAME rules stack the Map edits; toast + "rule" badge. Because a rule flips the path's `effectiveTarget`/`ruleOverridden` on the next enumeration, the badge + regroup + **revert** fall out of R1 for free — deleting the rule in the Map reverts the cells. Gate 1411; builderv3 76/76; probe `qzy-r9b-verify.mjs` 7/7 (prisma-seeded rule badges the 2 affected paths, delete reverts, net-zero via try/finally). **Deploy verify** `qzy-r9-deploy-verify.mjs` 12/12 on the live funnel-built fixture `cmr3ku9kb…` (2 real categories, 1152 paths): the picker lists the real buckets, Apply writes a rule targeting the picked category id with the full 6-answer path signature (real answer text), toast + rules-bar (real name) + "rule" badge, draft restored net-zero (never publishes) — closes the "picker needs a real catalog" gap. |
| R10 · Migration, probes, sweep | `2c190e2` | ✅ **COMPLETE + SHIPPED** — vocabulary sweep clean (the Logic UI uses result/rule/decider/override/dead-end/fallback; no legacy points-model terms) · architecture cross-link in `docs/quizzy-v1-architecture.md` (QZY-R row + probe inventory) · this status table current · probes per acceptance line (`qzy-r2…r9b-verify.mjs`). **DEPLOYED** — the full 26-commit batch (`1156d8c`…`2c190e2`) shipped via CI-on-push (run `28981018902`: gates + `flyctl deploy` + health + post-deploy smoke, no rollback). **Byte pin `c02ccaec98a0fe9e` HELD** on the live deploy — every runtime-touching phase (R5b/c · R6 · R7-2 · R7-3) preserved legacy-doc byte-identity. |

**Program status: COMPLETE.** All ten phases (R1–R10) shipped and live-verified;
the byte pin held across the entire batch. Remaining scoped follow-ups (not
blocking, documented in their rows): BT1 §2 full per-element inspector
*isolation* (R7) · step-chip inline drag-reroute in Paths (R8) — both have their
merchant-facing capability delivered; only the interaction polish is deferred.
