...
| R5 · Answer display + interaction states | (R5a `uncommitted`) | 🚧 decomposed into increments (largest phase + first runtime touch). **R5a ✅ (builder-only, zero runtime change):** radius unification (§3.1 — one control, chips snap the scrubber, legacy `shape` still resolves via `displayRadius`) + "Apply this option's look to all options" (§3.2, pure `copyOptionMediaToAll` + 3 tests). Gate green (1380 tests). Live builder verification blocked by the same fixture drift as R4 (needs a choice-Q; `task_f7fa65ee`). **R5b (next, RUNTIME touch — needs `/q` DOM-diff + screenshot):** independent media toggle (retire "Icon" as a layout mode), content-align L/C/R, separate image-size. **R5c:** BT6 interaction states (hover/selected config/motion presets/reveal-image via R4 picker/effects drawer) — rides R3 master/override. |
