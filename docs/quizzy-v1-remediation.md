...
| R9 · Table tab + override-writes-a-rule | R9-1 `279c496` | 🚧 **R9-1 ✅** (LV4) the **Table tab** (`LogicTableTab`) — same path dataset as a grid: collapsed = one row per result (result chip · path count · ✓/⚠N status); expand → per-path rows with a COLUMN per question, a skipped question rendering `–` (the complement of the Paths lanes); answer cells navigate to the Map. Builder-only read-only projection; result label reuses the Paths chain. Gate 1408; builderv3 76/76; probe `qzy-r9-verify.mjs` 11/11 + screenshot. **R9-2 (remaining):** result-cell **override → writes a path-signature rule** (generalize `draftRule` to multi-condition AND, append via `addDecisionRule`, toast + "rule" badge; deleting reverts) — reused by BOTH the Table and the Paths result chips. |
