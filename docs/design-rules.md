...
**The pictorial brand mark is the purple fox** (2026-08 rebrand; superseding this document’s earlier “no pictorial mark” stance, the interim “Q” monogram, and the Quartz cat). Shipped: `FoxMark` (the exact owner art, embedded via `brandAssets.ts`) inside `Wordmark`/`.qz-wordmark-mono`, beside the lowercase “wiskr” logotype with the whisker-flame accent over the i. The lockup is resolved (§14.1).
...
**14.1 Final logo lockup / wordmark.** ⟶ **RESOLVED (2026-08 rebrand):** the purple fox mark (`FoxMark`; closed eyes, long whiskers) beside the lowercase “wiskr” logotype (`Wordmark`), whisker-flame accent over the dotless i in brand purple. Owner-supplied art rendered exactly — masters in `docs/design/brand-2026/art/`, embedded via `brandAssets.ts`; favicon trio in `public/`.
