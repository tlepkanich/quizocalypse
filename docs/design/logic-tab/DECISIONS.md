...
## QWIDGET / WIS-026 (2026-09-08) — SUPERSEDED

> Kept as history. This was the Slack-bridge build of the same handoff (main `241a8d1`,
> deployed 2026-09-08). The 2026-09-09 merge replaced its widget, `RecommendationPicker.tsx`,
> its `.qz-qwidget-*` CSS and `e2e/qwidget-verify.mjs` with the build in the section below;
> the engine and guard decisions were the same in both.
...
|---|---|---|
| **1** | ~~One deciding answer points at exactly one recommendation.~~ **REVISED 2026-09-16 (owner, QWIDGET-M): a deciding answer may map SEVERAL recommendations.** `Answer.target_ids` lands with the exact G1 rule contract: `target_id` parses forever as the single form and mirrors `target_ids[0]`; length-1 normalizes back to `target_id` alone. | The mock's several-chips-per-cell IS built now: placing ADDS a chip (an occupied cell reads *Add {name}*, *{name} is already here* when present), each chip carries its own ×, picker rows are checkboxes that stay open. The engine flattens each selected answer's whole list into the §3 union; publish bakes every member; reconcile drops only dangling members; regenerate carries the list. Analytics/AI copy/path tools still read the anchor (same accepted gap as multi-target rules). |
