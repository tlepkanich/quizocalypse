...
// Legacy (points/ladder) docs only — the decider title/body overrides were
// retired in QL3-P5 (decider Continue gating lives in the v3 Step3Shell).
  ...
  return createPortal(
    <div className="qz-ql-guard-scrim" onMouseDown={onFix}>
      <div
        ...
        role="alertdialog"
        aria-modal="true"
        aria-label="Some recommendations have no mapped answers"
        onMouseDown={(e) => e.stopPropagation()}
      >
        <div className="qz-ql-guard-title">
          {`${n} ${n === 1 ? "recommendation has" : "recommendations have"} no mapped answers`}
        </div>
        {bucketNames.length > 0 ? (
          <ul className="qz-ql-guard-list">
            {/* Index keys: the list is a static snapshot, so index keys are
