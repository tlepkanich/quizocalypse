...
describe("applyDeciderQuestionFlow — review-hardened edges", () => {
  ...
  it("QWIDGET decision 2 — an all-multi-select quiz elects the multi-select as decider, uncoerced", () => {
    const seed = Quiz.parse({ ...buildSeedQuiz("Decider"), logic_model: "decider" });
    const allMulti = {
      questions: [
        {
    ...
    expect(q.data.question_type).toBe("multi_select"); // a multi-select may decide — not coerced
