...
AI surfaces re-export through the `app/lib/Codex.ts` barrel):
`MODEL = Codex-sonnet-4-6` for question builds/edits/research;
`MODEL_SPEED = Codex-haiku-4-5` for the funnel's type/template middle passes.
